...
fun FavouriteProductsScreen(
    viewModel: FavouriteProductsViewModel = hiltViewModel(),
    ...
    FavouriteProductsScreenContent(state = state, onEvent = viewModel::onEvent)
}

@OptIn(ExperimentalLayoutApi::class)
@Composable
private fun FavouriteProductsScreenContent(
    state: FavouriteProductsState,
    onEvent: (FavouriteProductsEvent) -> Unit
) {
    ...
    Scaffold(topBar = {
        TopAppBar(title = {
            ...
            navigationIcon = {
                IconButton(onClick = { onEvent(FavouriteProductsEvent.OnNavigateBack) }) {
            ...
            FlowRow(modifier = Modifier.padding(horizontal = dimensions.spaceMedium)) {
                state.products.forEachIndexed { index, product ->
                    ProductCard(
                        ...
                        onClick = {
                            onEvent(FavouriteProductsEvent.OnProductClick(product.id))
                        }
                    )
                }
            }
...
@Composable
@Preview
private fun FavouriteProductsScreenContentPreview() {
    EShopTheme {
        FavouriteProductsScreenContent(state = FavouriteProductsState(isLoading = true), onEvent = {})
    }
}